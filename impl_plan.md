...
4. **Presets (the "one model, many personas" feature)** — backend table for name/avatar/system-prompt/backend+model/sampling-params; frontend preset editor + switcher.
5. **Multimodal attachments** — backend reads model capabilities from the user-configured model configuration and gates the UI accordingly; frontend attachment upload, including edit-time add/remove (the one thing worth keeping from Cherry Studio).
